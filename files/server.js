...
app.post('/start-project', async (req, res) => {
  const { project_id, type, content, sampling_sec, user_agent_prompt } = req.body;
  const N8N_WEBHOOK_URL = process.env.N8N_WEBHOOK_URL;

  if (!N8N_WEBHOOK_URL) {
    return res.status(500).json({ error: 'N8N_WEBHOOK_URL non configurata nel .env' });
  }
...
server.listen(PORT, () => {
  console.log(`\n🚀 Antigravity Bridge in ascolto su http://localhost:${PORT}`);
